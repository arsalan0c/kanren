...
let three_turtles =
  ...
  let thrice (x, y, z) = conj_plus [ turtles x; turtles y; turtles z ] in
...
let fivesix_or_seven =
  ...
  let cond (x, y) =
    ifte (x === Atom (Int 5)) (y === Atom (Int 6)) (x === Atom (Int 7))
  in
...
let sat =
  ...
  let choices vars =
    List.map (fun x -> disj (boolean x true) (boolean x false)) vars
  in
  ...
  let disjunctions vars =
    [
      disj_plus
        [ boolean_l 0 vars true; boolean_l 1 vars false; boolean_l 2 vars true ];
      (* P \/ !Q \/ R*)
      disj_plus
        [ boolean_l 0 vars false; boolean_l 1 vars true; boolean_l 3 vars true ];
      (* !P \/ Q \/ S *)
      disj_plus [ boolean_l 1 vars true; boolean_l 3 vars false ];
      (* Q \/ !S *)
      disj_plus [ boolean_l 2 vars true; boolean_l 3 vars true ];
      (* R \/ S *)
      disj_plus [ boolean_l 0 vars true; boolean_l 2 vars false ];
      (* P \/ !R *)
    ]
  in
